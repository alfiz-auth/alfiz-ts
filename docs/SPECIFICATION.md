...
**The contract has three normative artifacts, and they cannot drift.** First, the `AlfizProvider` interface — the contract as a type, what a Client attaches to. Second, the `AlfizProviderBase` abstract class — the contract as an implementation root: exactly two kinds of provider extend it, the **local** provider (the Application, against its own database) and the **hosted** provider (an API connection to the contract served elsewhere — the attachment seam of the hosted dashboard, data-plane-less consumers, and federation). Third, the **Alfiz Provider API** — an OpenAPI document fixing the contract's wire form: one `POST {base}/v1/{op}` per operation, named-field JSON bodies, object results, and a typed-error envelope under which `not_org_root`, `graph_cycle`, `validation`, `not_found`, `conflict`, and `unsupported` survive the wire and re-throw identically on the consuming side. The interface and the operation manifest are held together at compile time; the manifest and the OpenAPI document at test time. The wire form is deliberately a language-agnostic document rather than a TypeScript export: a conforming provider — or consumer — in any implementation language is "the abstract class's surface, served (or consumed) over that API", which is what keeps the system portable beyond this reference implementation. Two contract members deliberately never cross the wire: the live `onInvalidate` stream (the epoch operations are the cross-process invalidation transport, §12.1) and the in-process ancestry function (an admin-only operation on the wire; runtime checks resolve ancestry locally, always).
...
Acyclicity is checked **transactionally**. Two concurrent edge insertions can each be individually cycle-free while jointly forming a cycle, so edge writes are serialized per graph via an advisory lock keyed on the graph (`"groups"`, `"reporting"`; one Application serves one organization, so the graph name is the whole key). These are administrative operations measured in writes per day; serialization is the correct simplicity tradeoff. Reporting edges (§6.4) are a tree under the same transactional enforcement. The storage seam owns the lock: the bundled drivers provide it (the Prisma driver requires a database advisory lock via `options.lock` for multi-node deployments), and the published driver conformance suite (`@alfiz/application/driver-suite`) holds custom drivers to the same serialization contract.
...
The Client ships headless administration logic rendering from the catalog and speaking only the provider contract. **Shipped today:** the wildcard-aware permission tree (`buildPermissionTree`, `toggleNode`, `isNodeChecked`, `isNodeIndeterminate` — whole-group selection stores the `<group>.*` pattern), plus the evaluation and workflow primitives every admin surface composes (`explain`, the request/policy functions, the provider contract itself). **Specified but not yet shipped:** the role editor, grant and revoke pickers, group membership editors, the request-and-approve surfaces (request form with justification prompts, approver queue), the view-as switcher, and a minimal unstyled reference assembly proving the kit composes end-to-end. Until those land, "buildable from the kit" means buildable from the tree logic and the contract — a real build, not an assembly job — and this document says so rather than implying otherwise. The design commitment is unchanged: the kit, when complete, withholds no workflow the hosted dashboard exposes. The hosted dashboard (§2.4) is the Service's assembled, styled, and maintained product, built from the identical components speaking the identical contract, attached directly to the Service — which serves federated data authoritatively and linked Applications by delegation (§2.7). The dashboard is therefore replaceable by construction: what is sold is assembly, polish, hosting, and seats, never a workflow the kit withholds. Components respect capability discovery, so surfaces for capabilities a provider lacks simply do not render — including org-domain editors rendering read-only against a provider that is not the org root.
